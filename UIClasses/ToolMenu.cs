...
namespace RinUI.UIClasses
{
    public class ToolMenu
    {
        ...
        {
            if (state.IsClicked && state.JudgeInAreaStartToEnd(new Vector2x2(X1, Y1, X2, Y2)))
            {
            ...
            if(_isOpen && state.IsClicked && !state.JudgeInAreaStartToEnd(new Vector2x2(X1, Y1, X2, Y2)))
            {
                _isOpen = false;
